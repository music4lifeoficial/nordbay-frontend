...
export default function UpgradeSuggestion({ className = "" }: { className?: string }) {
  ...
  const mt = t as any; // loose typing to avoid property errors while locale evolves
  return (
    <div className={`bg-yellow-50 border-l-4 border-yellow-400 p-4 flex items-center gap-3 rounded-md shadow-sm ${className}`}>
      ...
      <div className="flex-1">
        <div className="font-medium text-yellow-800">{mt?.mitid?.limitedAccess ?? "Limited access!"}</div>
        <div className="text-yellow-700 text-sm">{mt?.mitid?.suggestion ?? "Verify your identity with MitID to access all NordBay features."}</div>
      </div>
      <Link href="/auth/upgrade" className="ml-4 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-semibold px-3 py-2 rounded flex items-center gap-1 transition-colors">
        {mt?.mitid?.upgradeCta ?? "Upgrade account"} <ArrowRight className="w-4 h-4" />
      </Link>
    </div>
  );
}
