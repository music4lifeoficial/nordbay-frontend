...
export default function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true);
    if (!email || !password) {
      setFormError(t?.login?.missingFields || "Please complete all fields.");
      setIsLoading(false);
      return;
    }
    try {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-nordic-50 to-white px-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-nordic-900 mb-1">{t?.login?.title || "Sign in to NordBay"}</h1>
          <p className="text-nordic-600 mb-2">{t?.login?.subcopy || "Access your account and enjoy the platform."}</p>
        ...
        {/* Login Form */}
        <form onSubmit={handleSubmit} className="space-y-6" aria-label={t?.login?.formAria || t?.login?.submitAria || "Login form"}>
        ...
        <div className="mt-8 space-y-3">
          <div className="relative" aria-label={t?.login?.orContinueWith || "Or continue with"}>
            <div className="absolute inset-0 flex items-center">
              <div className="w-full border-t border-nordic-200" />
            </div>
            <div className="relative flex justify-center text-sm">
            <span className="bg-white px-2 text-nordic-600">{t?.login?.orContinueWith || "Or continue with"}</span>
            </div>
          </div>
          <button
            type="button"
            ...
            aria-label={t?.login?.googleAria || "Sign in with Google"}
          ...
          <button
            type="button"
            className="w-full flex items-center justify-center gap-2 border border-nordic-200 rounded-lg py-3 bg-white hover:bg-nordic-50 transition-colors"
            aria-label={t?.login?.facebookAria || "Sign in with Facebook"}
          ...
          <button
            type="button"
            className="w-full flex items-center justify-center gap-2 border border-nordic-200 rounded-lg py-3 bg-white hover:bg-nordic-50 transition-colors"
            aria-label={t?.login?.mitidAria || "Sign in with MitID"}
